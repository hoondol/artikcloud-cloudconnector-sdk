        ...
        switch (res.status) {
            case HTTP_OK:
                ...
                if (content == '' || content == 'OK') {
                    ...
                    if (req.url.startsWith("${ctx.parameters().endpoint}/me/feed")) {
    ...
    Or<ActionResponse, Failure> onAction(Context ctx, ActionDef action, DeviceInfo info) {
        switch (action.name) {
            case "sendToMe":
                ...
                def req = new RequestDef("${ctx.parameters().endpoint}/me/feed")
                              .withMethod(HttpMethod.Post)
                              .withContentType("application/x-www-form-urlencoded")
                              .withBodyParams([message: "ARTIK Cloud posted: " + valueToSend])
